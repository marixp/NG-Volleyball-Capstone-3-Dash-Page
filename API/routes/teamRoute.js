...
router.post ('/', async(req,res) => {
    try {
        const {teamName,captainUsername, users} = req.body;

        if (  !teamName || !captainUsername || !users ) {
        ...
        //Create Team
        const team = await Team.create({teamName,captainUsername});
...
router.get('/', async (request, response) => {
    try {
        // Fetch teams from the database and populate the 'users' field
        const teams = await Team.find({}).populate('users').populate();
